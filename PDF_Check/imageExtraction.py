...
pdfFile = wi(filename="Test2.pdf", resolution=300)
...
for page_number in range(len(open_file)):
    ...
    for image_number, img in enumerate(page.get_images(), start=1):
        xref = img[0]

        image_base = open_file.extract_image(xref)
        bytes_image = image_base["image"]

        ext_image = image_base["ext"]
        print(ext_image)
        print(image_base.keys())'''
